    ...
    renderButtonsGroup() {
        ...
        const onPressBuyNow = () => {
            if (this.inCartTotal < 5)
                ...
                alert('ProductLimitWaring');
        }

        const onPressDelete = () => {
            Alert.alert(
                'Confirm',
                'RemoveWishListItemConfirm',
                [
                    {text: 'CANCEL', onPress: () => undefined},
                    {text: 'YES', onPress: () => this.props.removeWishListItem(product, variation)},
                ]
            );
        }
        ...
        return (
            <TouchableOpacity onPress={callback} style={this.styles.itemWrapper}>
                <Icon icon={icon} size={25}
                      style={{color:Constants.Color.ViewBorder}}/>
            </TouchableOpacity>
        );
    }
}
